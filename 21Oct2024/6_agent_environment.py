...
class AgentEnvironment:
    ...
    def allocate_funds(self):
        """
        Each day, allocate funds by checking existing holdings. If we need to diversify holdings,
        proportionally sell from the main holding (e.g., NVDA) to generate cash for the additional stocks.
        """
        # Step 1: Calculate investment ratios based on predictions
        ratios = self.calculate_ratios()

        # Step 2: Identify stocks to sell if they have a zero ratio
        for stock in list(self.portfolio.keys()):
            if ratios.get(stock, 0) == 0 and self.portfolio[stock] > 0:
                # Sell all shares of the stock with zero target ratio
        ...
        # Step 3: Check if cash is enough for target ratios; if not, sell proportionally from main holdings
        total_portfolio_value = self.get_portfolio_value()
        target_allocation = {stock: total_portfolio_value * ratio for stock, ratio in ratios.items()}

        # Identify main holding if more than one stock needs to be bought
        stocks_to_buy = {stock: ratio for stock, ratio in ratios.items() if ratio > 0}

        if len(stocks_to_buy) > 1:
            # Identify the primary holding (largest value) from current portfolio
            primary_stock = max(self.portfolio, key=lambda stock: self.portfolio[stock] * self.current_prices[
                stock] if stock in stocks_to_buy else 0)

            # Calculate how much to sell from the primary stock to meet the target allocations
            for stock, ratio in stocks_to_buy.items():
                if stock != primary_stock:
                    # Calculate how much cash is needed for this stock
                    required_cash = total_portfolio_value * ratio
                    current_value = self.portfolio[primary_stock] * self.current_prices[primary_stock]
                    cash_needed = required_cash - (
                        self.portfolio[stock] * self.current_prices[stock] if stock in self.portfolio else 0)

                    # If more cash is needed, sell from primary stock
                    if cash_needed > 0 and current_value > cash_needed:
                        shares_to_sell = cash_needed // self.current_prices[primary_stock]
                        self.cash += shares_to_sell * self.current_prices[primary_stock]
                        self.log_action(primary_stock, 'sell', shares_to_sell, self.current_prices[primary_stock])
                        self.portfolio[primary_stock] -= shares_to_sell

        # Step 4: Reallocate funds based on the target allocation
        for stock, target_value in target_allocation.items():
            target_shares = target_value // self.current_prices[stock]
            shares_to_buy = target_shares - self.portfolio.get(stock, 0)

            # Buy shares if below target and if cash is available
            if shares_to_buy > 0 and self.cash >= shares_to_buy * self.current_prices[stock]:
                self.portfolio[stock] += shares_to_buy
                self.cash -= shares_to_buy * self.current_prices[stock]
                self.log_action(stock, 'buy', shares_to_buy, self.current_prices[stock])
